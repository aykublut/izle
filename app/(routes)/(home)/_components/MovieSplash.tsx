...
const MovieSplash = ({ movie }: { movie: any }) => {
  ...
  return (
    <Link onClick={handleCurrentMovie} href="/aboutMovie">
      <div className=" w-30 max-sm:h-50 sm:w-40 md:w-60 hover:scale-101 hover:border-b-[0.2px] cursor-pointer  border-white rounded-sm">
        <div className="flex justify-center items-center">
          <h2 className="text-sm md:text-2xl font-light p-1">{movie.name}</h2>
        </div>
        <div className="relative flex justify-center items-center w-30 max-sm:h-50 sm:w-40 md:w-60 md:h-88 h-59 rounded-md ">
          ...
          <Image
            ...
            src={movie.png}
            alt="wdawd"
            className="border-t-2 border-white rounded-md w-30 max-sm:h-50 sm:w-40 md:w-60 h-full"
            onLoadingComplete={() => setLoaded(true)}
          />
          <div className="absolute top-47 md:top-82 flex justify-center items-center flex-col gap-1">
            <Avatar
              className={
                movie.owner
